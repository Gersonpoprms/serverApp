...
export const getEvents = async (req, res)=>{
    var data ={type:req.params.type , IdFormat_FK:req.params.IdFormat_FK , idEq:req.params.idEq}
...
export const deleteEventById = async (req, res)=>{
    ...
    try {
        ...
        if(result.affectedRows)
            res.json({status:200, data:{}, msj:"evento borrado con exito"});
        else
...
export const undeleteEvent = async (req, res)=>{
    ...
    try {
        ...
        if(result.affectedRows)
            res.json({status:200, data:{}, msj:"evento restaurado con exito"});
        else
            res.json({status:204, data:{}, msj:error.sqlMessage});     
    } catch (error) {
            console.log(error)
...
export const updateEvent = async (req, res)=>{
    ...
    try {
        ...
        if(result.affectedRows)
            res.json({status:200, data:{}, msj:"evento actualizado con exito"});
        else
            res.json({status:204, data:{}, msj:error.sqlMessage}); 
    } catch (error) {
            console.log(error)
